...
```mermaid
flowchart TD
    A["🧭 Autonomy<br/>(Tự chủ)"]
    B["💪 Competence<br/>(Năng lực)"]
    C["🤝 Relatedness<br/>(Kết nối)"]
    D["✨ Intrinsic Motivation<br/>(Động lực bền vững)"]
...
flowchart TD
    subgraph Controlled["🔴 Controlled"]
        direction LR
        E["External<br/>(Bị bắt / được thưởng)"]
        IJ["Introjected<br/>(Guilt / xấu hổ)"]
    end

    subgraph Autonomous["🟢 Autonomous"]
        direction LR
        ID["Identified<br/>(Thấy có giá trị)"]
        IG["Integrated<br/>(Là con người mình)"]
        IN["Intrinsic<br/>(Tự nó thú vị)"]
    end

    E -->|internalization| IJ
    IJ -->|internalization| ID
    ID -->|internalization| IG
    IG -->|internalization| IN

    style E fill:#fee2e2,stroke:#dc2626,color:#7f1d1d
    style IJ fill:#fecaca,stroke:#dc2626,color:#7f1d1d
    style ID fill:#d1fae5,stroke:#059669,color:#064e3b
    style IG fill:#a7f3d0,stroke:#059669,color:#064e3b
    style IN fill:#6ee7b7,stroke:#059669,color:#064e3b
```
